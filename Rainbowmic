...
-- Function to check for "GradientMic" in the player's inventory
local function hasGradientMic()
    for _, item in pairs(player.Backpack:GetChildren()) do
        if item.Name == "GradientMic" then
            return true
        end
    end
    return false
...
local function fireOtherPlayersPrompts()
    ...
    -- Fire proximity prompts until "GradientMic" is obtained or the script is deactivated
    while not hasGradientMic() and scriptActive do
        local promptsToFire = {}
        ...
        -- Check if "GradientMic" has been obtained
        if hasGradientMic() then
            print("GradientMic obtained! Stopping script.")
            scriptActive = false
            break
        end
