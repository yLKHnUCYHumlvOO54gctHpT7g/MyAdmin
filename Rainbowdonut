...
-- Function to check for "GradientDonut" in the player's inventory
local function hasGradientDonut()
...
local function fireOtherPlayersPrompts()
    ...
    -- Fire proximity prompts until "GradientDonut" is obtained or the script is deactivated
    while not hasGradientDonut() and scriptActive do
        local promptsToFire = {}
        ...
        -- Check if "GradientDonut" has been obtained
        if hasGradientDonut() then
            print("GradientDonut obtained! Stopping script.")
            scriptActive = false
            break
        end
